...
When launching an `implementation-executor` agent for a fix, you **must** use `subagent_type: "fortran-development-pipeline:implementation-executor"` in the Agent tool call — never use the default general-purpose agent. Using the wrong agent type means the coding standards and quality gates defined in the `implementation-executor` agent definition will not be applied.
...
> **Critical**: The Agent tool call must always include `subagent_type: "fortran-development-pipeline:implementation-executor"` and `name: "{ISSUE_ID}"`. The `name` field makes the agent addressable via SendMessage and is used by the SubagentStop hook to locate the correct sidecar. Example skeleton:
>
> ```
> Agent({
...
3. **Execute tasks sequentially — one at a time**:
   - **CRITICAL**: For EVERY task, you MUST launch the `implementation-executor` **agent** (via the Agent tool with `subagent_type: "fortran-development-pipeline:implementation-executor"`). Do NOT implement fixes yourself inline.
   ...
   - Complete each task fully before moving to the next:
     1. Launch `implementation-executor` agent using the **Agent Prompt Template** above. Set `subagent_type: "fortran-development-pipeline:implementation-executor"`, pass the script path, document path, issue ID, and compiled script path (`<fix-doc-dir>/compiled/<ISSUE-ID>.sh`). Do NOT copy fix content into the prompt; the agent runs the compiled script directly.
...
- For Fortran code changes: Check LSP diagnostics first, then compile with `gfortran -c -Wall <file>.f90` or the project build command
...
3. Use `LSP hover` to verify subroutine signatures match expectations after the fix
...
- **Ignore line numbers as addresses** — if the fix document references `file.f90:42`, treat `42` as a hint only. Re-locate the target by searching for the surrounding code context (unique string, subroutine name, or LSP symbol) before each edit.
