...
3. **Execute tasks sequentially — one at a time**:
   - **CRITICAL**: For EVERY task, you MUST launch the `implementation-executor` **agent** (via the Agent tool with `subagent_type: "fortran-dev-pipeline:implementation-executor"`). Do NOT implement fixes yourself inline.
